...
cat ~/.pass | sudo -S apt install -y nala git >/dev/null && echo "Installed nala and git!"
...
cat ~/.pass | sudo -S curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh | bash >/dev/null && echo "Installed Brew!"
...
mkdir -p $HOME/.config
cat ~/.pass | sudo -S cp $HOME/dotfiles/fonts/* /usr/local/share/fonts/ && echo "Copied Fonts!"
cat ~/.pass | sudo -S sed -i 's/GRUB_TIMEOUT=5/GRUB_TIMEOUT=0/' /etc/default/grub && sudo update-grub > /dev/null && echo "Updated Grub!"
#sudo echo "export EDITOR=$(which nvim)" >> /root/.profile && echo "root EDITOR is set to nvim!"
cat ~/.pass | sudo -S echo "$(which fish)" >> /etc/shells && echo "Added fish to /etc/shells!"
